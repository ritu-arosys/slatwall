<cfcomponent>
	<cfscript>
		...
		public function updateTestData(){
			...
			updateTestRunnerAccountAuthCreatedDateTime();
		}
	</cfscript>
	
	<cffunction name="updateTestRunnerAccountAuthCreatedDateTime">
		<cfset var rs = "" />
		<cfquery name="rs">
			UPDATE SwAccountAuthentication SET createdDateTime = <cfqueryparam cfsqltype="cf_sql_timestamp" value="#now()#" /> WHERE accountAuthenticationID = 'a6aeb9a7f8b84bb4a7ab7e49805a512d'
		</cfquery>
	</cffunction>
