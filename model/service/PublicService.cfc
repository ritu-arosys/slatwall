...
component extends="HibachiService" accessors="true" output="false" hint="Contains all public API scope contexts"
{
	...
	//variables.requestHeaders = getHTTPRequestData();
	...
	any function login( required struct rc ){
		...
		if (rc.apiRequest){
			...
			try {
				if (StructKeyExists(arguments.rc.apiRequestHeaders, "authorization")){
					var plaintextArray = ListToArray( ToString( ToBinary( arguments.rc.apiRequestHeaders.authorization ) ), ":" );
			...
			return {
				entityService = getService('accountService'),
				entity = arguments.rc.$.slatwall.getAccount()
			};
		}else{
	...
	public any function createAccount( required struct rc ) {
		...
		if (rc.apiRequest && StructKeyExists(arguments.rc.apiRequestHeaders, "emailAddress")){
			try {
			arguments.rc["firstName"] = arguments.rc.apiRequestHeaders.lastName;
			arguments.rc["lastName"] = arguments.rc.apiRequestHeaders.lastName;
			arguments.rc["company"] = arguments.rc.apiRequestHeaders.company;
			arguments.rc["phone"] = arguments.rc.apiRequestHeaders.phone;
			arguments.rc["emailAddress"] = arguments.rc.apiRequestHeaders.emailAddress;
			arguments.rc["emailAddressConfirm"] = arguments.rc.apiRequestHeaders.emailAddressConfirm;
			arguments.rc["password"] = arguments.rc.apiRequestHeaders.password;
			arguments.rc["passwordConfirm"] = arguments.rc.apiRequestHeaders.passwordConfirm;
			arguments.rc["createAuthenticationFlag"] = arguments.rc.apiRequestHeaders.createAuthenticationFlag;
			} catch (any e){
	...
	*/
	private void function addDataToResponse(string key, data, required struct rc){
		//Populate it with the data as JSON
		arguments.rc.apiResponse.content[key] = arguments.data;
	}
