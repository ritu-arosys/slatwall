<cfcomponent output="false" accessors="true" extends="Slatwall.org.Hibachi.HibachiController">
	...
	<cffunction name="ngslatwall" >
		...
		<cfset var dtExpires = (Now() + 60) />
		 
		<cfset var strExpires = GetHTTPTimeString( dtExpires ) />
			...
			<cfsavecontent variable="local.jsOutput">
				<cfoutput>
					...
					function(){
								...
								<cfloop array="#rc.entities#" index="local.entity">
									...
									<cftry>
										...
										_jsEntities[ '#local.entity.getClassName()#' ]=function() {
											...
											this.validations = #serializeJSON(rc.$.slatwall.getService('hibachiValidationService').getValidationStruct(local.entity))#;
											...
											this.metaData.$$getDetailTabs = function(){
											    ...
											    />
											    var detailTabs = [
											    	<cfset local.tabCount = 0 />
											    	<cfloop query="local.tabsFileList">
											    		<cfset local.tabCount++ />
											    		
											    		<cfif local.tabCount neq local.tabsFileList.recordCount>
											    			{
											    				tabName:'#name#'
											    			},
											    		<cfelse>
											...
											<cfif len(rc.$.slatwall.getService('attributeService').getAttributeCodesListByAttributeSetObject(local.entity.getClassName()))>
												<cfloop list="#rc.$.slatwall.getService('attributeService').getAttributeCodesListByAttributeSetObject(local.entity.getClassName())#" index="local.attributeCode">
						...
						/* $slatwallProvider.setConfigValue(rc.$.slatwall.getConfig().baseURL); */
	...
	<cffunction name="ngcompressor">
		...
		<cfif !request.slatwallScope.hasApplicationValue('ngCompressor_#hash(arguments.rc.jspath)#')>
			...
			<cfif request.slatwallScope.getApplicationValue('debugFlag')>
				<cfset getPageContext().getOut().clearBuffer() />
				<cfset request.slatwallScope.setApplicationValue('ngCompressor_#hash(arguments.rc.jspath)#',local.jsOutput)>
			<cfelse>
				<cfset getPageContext().getOut().clearBuffer() />
				<cfset local.oYUICompressor = createObject("component", "Slatwall.org.Hibachi.YUIcompressor.YUICompressor").init(javaLoader = 'Slatwall.org.Hibachi.YUIcompressor.javaloader.JavaLoader', libPath = expandPath('/Slatwall/org/Hibachi/YUIcompressor/lib')) />
		...
		<cfelse>
			<cfset local.jsOutput = request.slatwallScope.getApplicationValue('ngCompressor_#hash(arguments.rc.jspath)#')>
		</cfif>
